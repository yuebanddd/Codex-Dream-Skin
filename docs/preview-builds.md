...
主题应用使用分阶段日志定位渲染器故障：`cdp_theme_stage_started` 记录载荷大小和摘要，`cdp_theme_art_progress` 记录图片分片里程碑，`cdp_theme_install_queued` 表示异步安装已经排队，`cdp_theme_install_phase` 记录校验、组装和挂载阶段，最终由 `cdp_theme_install_completed` 或 `cdp_theme_install_failed` 收口。若安装排队后渲染器失去响应，`cdp_theme_install_terminal` 或 `watcher_install_terminal` 会带有 `retrySuppressed: true`，表示客户端已主动停止自动回退和重复注入。
