...
PR、`release` 分支 push 和手动 `workflow_dispatch` 都会构建测试包。Artifact 保留 14 天，不会自动创建 GitHub Release。
...
如有问题，点击“导出 JSON”。导出内容包含本机 Codex 可执行文件路径，公开提交前应先检查并移除不希望披露的用户名或目录信息。不要上传 API Key、`auth.json`、对话内容或私人截图。
